...
- `aliases/utils/internal`, `aliases/utils/external` → the bulk of personal aliases / functions (git, Go, password gen, `gitloc`, etc.). `external` is the largest file and the most-edited. **`claudio` (two-account Claude Code launcher) lives here; before touching it or anything Claude-auth-related, read `notebook.d/claudio.md` — it documents the per-`CLAUDE_CONFIG_DIR` Keychain-slot model (how two full-scope logins stay isolated), and a full Claude OAuth reference (endpoints, scopes, the Console-vs-subscription authorize gotcha). Easy to break in a refactor.**
