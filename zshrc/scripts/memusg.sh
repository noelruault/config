...
#
# This is a copy of https://gist.github.com/netj/526585#file-memusg and below
# license applies.
#
...
*)
    echo "$(uname): unsupported operating system" >&2
    exit 2
    ;;
esac
...
(
    peak=0
    while sizes=$(sizes $pgid); do
        set -- $sizes
        sample=$((${@/#/+}))
        let peak="sample > peak ? sample : peak"
        sleep 0.1
    done
    echo "\nMaximum peak RAM usage: $peak" >&2
) &
monpid=$!
